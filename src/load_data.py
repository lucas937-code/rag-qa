...
import os
from config.paths import HF_CACHE_DIR, TRAIN_DIR, VAL_DIR, TEST_DIR
...
def download_and_prepare_data():
    download_config = DownloadConfig(cache_dir=HF_CACHE_DIR)
    ...
    for i, ex in enumerate(stream):
        if i < VAL_SPLIT_SIZE:
            ...
            if len(val_buffer) >= BATCH_SIZE:
                save_shard(val_buffer, VAL_DIR, val_idx)
                val_buffer, val_idx = [], val_idx + 1
        else:
            train_buffer.append(ex)
            if len(train_buffer) >= BATCH_SIZE:
                save_shard(train_buffer, TRAIN_DIR, train_idx)
